    ...
    for code in constants.CODES:
        for label in constants.LABELS[38:47]:
            ...
            for t_count, transformation in enumerate(get_random_transformations()):
                # Video output path
                video_path = f'{constants.MPI_WONE_AUGMENTED_DATASET_PATH}/{label}/{code}_{label}_{t_count:02d}.avi'
                ...
                os.makedirs(f'{constants.MPI_WONE_AUGMENTED_DATASET_PATH}/{label}', exist_ok=True)
                out = cv2.VideoWriter(
                    video_path,
                ...
                for i_count in range(start, stop):
                    ...
                    if i_count == 0:
                        cv2.imwrite(f'{constants.MPI_WONE_AUGMENTED_DATASET_PATH}/{label}/transformation_oo.png', x)
                    ...
                    if i_count == 0:
                        cv2.imwrite(f'{constants.MPI_WONE_AUGMENTED_DATASET_PATH}/{label}/transformation_{t_count:02d}.png', x)
